...
def mlnTest2t(a, ab, ac, abcd):
    ...
    st = 1.
    if ab * ac < a * abcd:
        for i in range(min(int(round(a-1)), int(round(ab * ac / abcd))), int(round(a_min))-1, -1):
            pi = lgamma(i+1) + lgamma(ab-i+1) + lgamma(ac-i+1) + lgamma(abcd-ab-ac+i+1)
            if pi < pa: continue
            st_new = st + exp(pa - pi)
            if st_new == st: break
            st = st_new
        for i in range(int(round(a))+1, int(round(a_max))+1):
            pi = lgamma(i+1) + lgamma(ab-i+1) + lgamma(ac-i+1) + lgamma(abcd-ab-ac+i+1)
            st_new = st + exp(pa - pi)
            if st_new == st: break
            st = st_new
    else:
        for i in range(int(round(a))-1, int(round(a_min))-1, -1):
            pi = lgamma(i+1) + lgamma(ab-i+1) + lgamma(ac-i+1) + lgamma(abcd-ab-ac+i+1)
            st_new = st + exp(pa - pi)
            if st_new == st: break
            st = st_new
        for i in range(max(int(round(a))+1, int(round(ab * ac / abcd))), int(round(a_max))+1):
            pi = lgamma(i+1) + lgamma(ab-i+1) + lgamma(ac-i+1) + lgamma(abcd-ab-ac+i+1)
            if pi < pa: continue
            st_new = st + exp(pa - pi)
            if st_new == st: break
