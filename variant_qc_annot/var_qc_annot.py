...
def calc_stats(alleles, probs, fil, controls, hardy_fil, ad):
    ...
    if alleles == 1:
        ...
        incoef, hardypval = hardy(probs_fil[0,hardy_fil].sum(), probs_fil[1,hardy_fil].sum(), probs_fil[2,hardy_fil].sum())
        control_incoef, control_hardypval = hardy(probs_fil[0,control_fil].sum(), probs_fil[1,control_fil].sum(), probs_fil[2,control_fil].sum())
    ...
    if ac_het == 0:
        ...
        d_all = (dref_ref + dref_alt + dhet_ref + dhet_alt + dhom_ref + dhom_alt) / (ac_ref + ac_het + ac_hom)
    ...
    return (filfrac, ac, gt, dpref, dpalt, abhet, abhom, dhet_frac, abdev_rate, incoef, -numpy.log10(hardypval) * 10.0, control_incoef, -numpy.log10(control_hardypval)* 10.0, althet, total_althet)
...
def process_row(args):
    ...
    det_stats_raw_d10 = calc_stats(alleles, pl01, fil_d10, control_fil, hardy_fil, adrow)
    det_stats_raw_gq20 = calc_stats(alleles, pl01, fil_gq20, control_fil, hardy_fil, adrow)
    det_stats_p = calc_stats(alleles, posterior_cor, None, control_fil, hardy_fil, adrow)
    det_stats_p_d10 = calc_stats(alleles, posterior_cor, fil_d10, control_fil, hardy_fil, adrow)


    result = (chrom, pos, ref, alt) + det_stats_raw_d0 + det_stats_raw_d6 + det_stats_raw_d10 + det_stats_raw_gq20 + det_stats_p + det_stats_p_d10
    return result
...
def parse_matrix(outfile,meta_file, filename, range_description=None, nproc=1):
    ...
    control_samples = set([iid for iid, status in zip(meta['iid'], meta['status']) if status == '0'])
    female_samples = set([iid for iid, gender  in zip(meta['iid'], meta['gender']) if gender == '2'])
    male_samples = set([iid for iid, gender  in zip(meta['iid'], meta['gender']) if gender == '1'])

    
    #open out file and write header
    outf = open(outfile, 'w')
    fieldnames = ['chrom','pos', 'ref', 'alt']
    res = ['kept','ac', 'gt','dpref','dpalt', 'abhet','abhom','fdhet','abdev', 'incoef', 'hw', 'control_incoef','control_hw', 'althet', 'total_althet']
    ...
    female_control_fil = numpy.array([a in control_samples and a in female_samples for a in samplenames],dtype=bool)
    female_fil = numpy.array([a in female_samples for a in samplenames],dtype=bool)
    male_control_fil = numpy.array([a in control_samples and a in male_samples for a in samplenames],dtype=bool)
    ...
    fo = '%s\t%d\t%s\t%s' + '\t%f\t%f\t%s\t%s\t%s\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f' * 6 + '\n'
    while rows:
        if nproc == 1:
            for row in rows:
            ...
            res = pool.map_async(vqa.process_row, [e + (control_fil,hardy_fil) for e in rows], 50)
