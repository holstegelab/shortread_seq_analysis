...
vcf_reader.add_format_to_header({'ID':"DS", 'Number': '1', 'Type':'Float', 'Description':'Dosage (posterior)'})
...
for v in vcf_reader:
    ...
    if pls.T.shape[0] == 3 or pls.T.shape[0] == 2:
        if threshold == -1:
            ...
            posterior_pls = numpy.zeros((tposterior_pls.shape[0], len(depth)),dtype=float)
    ...
    if threshold >= 0: #for genotypes with depth < threshold, set genotype quality to 0 (missing genotype).
        gq[~fil] = 0
