...
adapter_finder = re.compile(
    "AAAAAAAA|AGATCGGAAGAGC|AATGATACGGCGACCACC|CAAGCAGAAGACGGCA|AGATGTGTATAAGAGACAG"
)
    ...
    for tag in tags:
        if tag.startswith("SA"):
            ...
            for srec in tag[5:].split(";")[:-1]:
                ...
                w[3] = "".join(
                    [
                        "%d%s" % (length, "H" if ctype == "S" else ctype)
                        for ctype, length in sc
                    ]
                )
...
def rg_hardclip(readgroup, update_mate_tag):
    ...
    readgroup["supplementary"] = [
        record_hardclip(r, update_mate_tag) for r in readgroup["supplementary"]
    ]
    return readgroup
...
def rg_prune(readgroup):
    ...
    sequse = numpy.zeros(
        len(readgroup["primary"][9]), dtype=int
    )  # total aligncount per base
        ...
        if mode == "clips":
            ...
            with sys.stdout as f2:
                for row in reader:
                    ...
                    softclipped, aligned, inserted, deleted = soft_clipped_region(
                        scigar
                    )
                    ...
                    for ctype, length in scigar:
                        if ctype == "S":
                            ...
                            ctype == "H" or ctype == "D" or ctype == "N" or ctype == "P"
                        ):
                            pass
                    f2.write(
                        "%s\t%d\t%d\t%d\t%d\t%d\t%s\n"
            ...
            with sys.stdout as f:
                ...
                w.writerow(
                    (
                        ...
                        "sup_diffchrom_ratio",
                        "duplicate_ratio",
                        "duplicate_supplement_ratio",
                        "soft_clipped_bp_ratio",
                        "aligned_bp_ratio",
                        "inserted_bp_ratio",
                        "deleted_bp_ratio",
                        "total_bp",
                        "soft_clipped_bp_ratio_filter_50",
                        "soft_clipped_bp_ratio_filter_60",
                        "soft_clipped_bp_ratio_filter_70",
                        "aligned_bp_ratio_filter_50",
                        "aligned_bp_ratio_filter_60",
                        "aligned_bp_ratio_filter_70",
                        "poly_a",
                        "illumina_adapter",
                        "pcr_adapter_1",
                        "pcr_adapter_2",
                ...
                w.writerow(
                    (
                        float(unmapped_count) / float(total_count),
                        mqual20_count / float(total_count),
                        secondary_count / float(total_count),
                        supplementary_count / float(total_count),
                        float(sup_diffchrom_count)
                        / max(float(supplementary_count), 1.0),
                        duplicate_count / float(total_count),
                        duplicate_supplement_count
                        / max(float(supplementary_count), 1.0),
                        softclipped_base_count / float(base_count),
                        aligned_base_count / float(base_count),
                        inserted_base_count / float(base_count),
                        deleted_base_count / float(base_count),
                        base_count,
                        (softclipped_base_count - soft_clipped_50) / float(base_count),
                        (softclipped_base_count - soft_clipped_60) / float(base_count),
                        (softclipped_base_count - soft_clipped_70) / float(base_count),
                        (aligned_base_count - aligned_50) / float(base_count),
                        (aligned_base_count - aligned_60) / float(base_count),
                        (aligned_base_count - aligned_70) / float(base_count),
                        poly_a / float(total_count),
                        illumina_adapter / float(total_count),
                        pcr_illumina_adapter_1 / float(total_count),
                        pcr_illumina_adapter_2 / float(total_count),
                        nextera / float(total_count),
                    )
                )
                    ...
                    for row in reader:
                        ...
                        if lastname == name:
                            ...
                            if querygroup:
                                ...
                                res_filtered = (
                                    "\n".join(
                                        ["\t".join(xrow) for xrow in res_filtered]
                                    )
                                    + "\n"
                                )
